...
amixer=$(amixer -D pulse | grep "Playback" | tail -n 1)
percent="${amixer%\ *}"
percent="${percent##*\ }"
percent="${percent//\%/}"
percent="${percent//\[/}"
...
txt=$(round $percent)
echo "<img>$image</img>\
      <txt><span weight='bold' fgcolor='$color'>$txt</span></txt>\
      <click>amixer -D pulse set Master toggle</click>"
